  ...
  "node_modules/pngjs/lib/bitmapper.js"(exports) {
    ...
    var pixelBppMapper = [
      ...
      function(pxData, data, pxPos, rawPos) {
        ...
        let pixel2 = data[rawPos];
        pxData[pxPos] = pixel2;
        pxData[pxPos + 1] = pixel2;
        pxData[pxPos + 2] = pixel2;
        pxData[pxPos + 3] = 255;
      },
      // 2 - LA
      // 0: 0, 1: 0, 2: 0, 3: 1
...
function upscale(source) {
  ...
  const output = new import_pngjs.PNG({ width: source.width * factor, height: source.height * factor });
  for (let y = 0; y < output.height; y++)
    for (let x = 0; x < output.width; x++) {
      const sx = Math.floor(x / factor), sy = Math.floor(y / factor);
      const si = (source.width * sy + sx) * 4, di = (output.width * y + x) * 4;
...
function renderOverlay(input2, score = {}) {
  const source = import_pngjs.PNG.sync.read(input2);
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      ...
      if (response.status === 429) {
        ...
        retry429 = false;
        const wait = Math.min(3e4, Number(response.headers.get("retry-after") || 1) * 1e3);
